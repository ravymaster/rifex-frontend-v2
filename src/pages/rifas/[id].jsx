    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'grid', placeItems: 'center', zIndex: 50 }}>
      <form onSubmit={onSubmit} style={{ background: '#fff', borderRadius: 16, padding: 20, width: 'min(560px, 92vw)', border: '1px solid #E5E7EB' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          ...
          <ActionButton tone="default" type="submit">
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'grid', placeItems: 'center', zIndex: 50 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 20, width: 'min(520px, 92vw)', border: '1px solid #E5E7EB' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <ActionButton tone="danger" onClick={doClose}>
            {busy ? 'Cerrando…' : 'Terminar ahora'}
          </ActionButton>
        </div>
      </div>
    </div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'grid', placeItems: 'center', zIndex: 60 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 20, width: 'min(520px, 92vw)', border: '1px solid #E5E7EB' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <ActionButton tone="danger" onClick={doDelete}>
...
export default function Panel() {
  ...
  const isUUID = (s) => /^[0-9a-fA-F-]{36}$/.test(String(s || '').trim());
  ...
  async function load() {
    ...
    try {
      ...
      // 2) query base (RLS limita a tus rifas)
      let query = supabase
        .from('raffles')
        .select('id, title, status, price_cents, total_numbers, end_date, created_at')
      ...
      // 3) contar vendidos por rifa
      async function countSold(raffleId) {
      ...
      for (const r of raffles || []) {
        const sold = await countSold(r.id);
        items.push({ ...r, sold });
      }
      ...
      <main className="container" style={{ padding: '24px' }}>
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <ActionButton tone="ghost" onClick={load}>
              Refrescar
            </ActionButton>
            <a href="/crear-rifa">
              <ActionButton>Crear rifa</ActionButton>
            </a>
          </div>
        </header>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 12, marginBottom: 16 }}>
          <Kpi label="Recaudado (CLP)">
            <PesoCLP cents={totals.revenueCents} />
          </Kpi>
            ...
            >
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  <Progress value={row.sold} max={row.total_numbers || 1} />
                </div>
              </div>
              <div>
                <Badge tone={row.status === 'active' ? 'green' : row.status === 'closed' ? 'red' : 'gray'}>{row.status}</Badge>
              </div>
              <div>
                {row.sold}/{row.total_numbers}
              </div>
              <div>
                <PesoCLP cents={row.price_cents} />
              </div>
              <div>{row.end_date || '—'}</div>
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <a href={`/rifas/${row.id}`}>
                  <ActionButton tone="ghost">Ver</ActionButton>
                </a>
                <ActionButton tone="ghost" onClick={() => setEditRaffle(row)}>
                  Editar
                </ActionButton>
                {row.status !== 'closed' && (
                  <ActionButton tone="danger" onClick={() => setCloseRaffle(row)}>
                    Terminar
                  </ActionButton>
                )}
                <ActionButton tone="danger" onClick={() => setDeleteRaffle(row)}>
                  Eliminar
                </ActionButton>
              </div>
            </div>
