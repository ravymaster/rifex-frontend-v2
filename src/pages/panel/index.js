...
// Enlace con aspecto de botón (para evitar <button> dentro de <a>)
function LinkButtonGhost({ href, children }) {
  const style = { background: '#fff', color: '#111827', border: '1px solid #E5E7EB', padding: '8px 12px', borderRadius: 8, fontWeight: 600, display: 'inline-block', textDecoration: 'none' };
  return (
...
function EditModal({ open, onClose, raffle, onSaved }) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      alert(err.message || 'update_failed');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'grid', placeItems: 'center', zIndex: 50 }}>
      <form onSubmit={onSubmit} style={{ background: '#fff', borderRadius: 16, padding: 20, width: 'min(560px, 92vw)', border: '1px solid #E5E7EB' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          ...
          <ActionButton tone="default" type="submit">{busy ? 'Guardando…' : 'Guardar cambios'}</ActionButton>
        </div>
      </form>
    </div>
  );
...
function CloseDialog({ open, onClose, raffle, onClosed }) {
  ...
  async function doClose() {
    ...
    try {
      ...
      alert(err.message || 'close_failed');
    } finally {
      setBusy(false);
    }
  }
...
export default function Panel() {
  ...
  // (NUEVO) Intercambia el code/hash cuando vienes desde email/Google y limpia la URL.
  useEffect(() => {
    (async () => {
      try {
        const href = typeof window !== 'undefined' ? window.location.href : '';
        const hasOAuthStuff =
          href.includes('code=') || href.includes('access_token') || href.includes('refresh_token');

        if (hasOAuthStuff) {
          await supabase.auth.exchangeCodeForSession(href);
          // Limpia la URL para evitar repetir el exchange en refresh
        ...
        console.warn('[panel] exchangeCodeForSession:', e?.message || e);
      } finally {
        // siempre cargamos el panel después del exchange (o si no había nada que intercambiar)
        load();
      }
    })();
  ...
  // (NUEVO) Bootstrap de perfil al primer login (idempotente)
  useEffect(() => {
    (async () => {
          ...
          .upsert(
            {
              ...
              rut: user.user_metadata?.rut_beneficiario || null,
            },
            { onConflict: 'user_id' }
          );
      } catch (e) {
  ...
  async function load() {
    ...
    try {
      ...
      catch { throw new Error(`HTTP ${r.status} ${r.statusText}`); }
  ...
  // Si quieres mantener también tu primera carga “clásica”, déjalo:
  // useEffect(() => { load(); }, []); // (opcional) causará doble fetch la 1ª vez
  ...
  return (
    <Layout>
      <main className="container" style={{ padding: '24px' }}>
        ...
        {noSession && (
          <div style={{ margin: '0 0 16px', padding: '10px 12px', background: '#FEF3C7', border: '1px solid #FDE68A', color: '#92400E', borderRadius: 10 }}>
            Debes <a href="/login" style={{ fontWeight: 700, color: '#92400E' }}>iniciar sesión</a> para ver y gestionar tus rifas.
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
          {['all', 'active', 'draft', 'closed', 'deleted'].map((s) => (
            <button
              key={s}
              onClick={() => { setStatus(s); setTimeout(load, 0); }}
              style={{
                padding: '6px 10px',
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            <input
              ...
              style={{ padding: '8px 12px', border: '1px solid #E5E7EB', borderRadius: 10, minWidth: 260 }}
            />
        ...
        {/* Tabla */}
        <section data-panel="list" style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, overflow: 'hidden' }}>
          <div
            data-panel="hdr"
            style={{
              display: 'grid',
              gridTemplateColumns: '1.4fr 110px 160px 120px 140px 1fr',
              gap: 0,
              padding: '12px 16px',
              fontSize: 12,
              color: '#6B7280',
      ...
      <style jsx global>{`
        ...
        @media (max-width: 640px) {
          ...
          main.container [data-panel="hdr"]{ display: none !important; }

          main.container [data-panel="row"]{
          ...
          main.container [data-panel="row"] > div{ margin-bottom: 2px; }

          main.container [data-panel="row"] [data-cell="actions"]{
            justify-content: flex-start !important;
            flex-wrap: wrap !important;
            gap: 8px !important;
            margin-top: 6px;
          }
          main.container [data-panel="row"] [data-cell="actions"] a,
          main.container [data-panel="row"] [data-cell="actions"] button{
