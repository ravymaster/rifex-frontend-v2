...
function getParams() {
  ...
  // Soportar ambos esquemas de MP:
  //  a) payment_id, status, preference_id
  //  b) collection_id, collection_status, preference_id
  const payment_id =
  ...
  const status =
    ...
    u.searchParams.get('collection_status'); // approved | pending | rejected
    ...
    <div style={{ maxWidth: 640, margin: '2rem auto', fontFamily: 'sans-serif' }}>
      ...
      {state === 'error' && (
        <div style={{ marginTop: 12 }}>
          ...
          <button onClick={doConfirm} style={{ padding:'8px 12px', border:'1px solid #ccc', borderRadius:6 }}>
            Reintentar confirmación
          </button>
          {resp?.error && (
            <pre style={{ whiteSpace:'pre-wrap', background:'#f7f7f7', padding:10, borderRadius:6, marginTop:10 }}>
              {String(resp.error)}
            </pre>
          )}
        </div>
