...
> **2026-09-04 addendum (most recent) — RIFEX INSCRIPCIONES V1 FREE + FUTURE BILLING FOUNDATION PROD PROMOTION:** `origin/main`/PROD avanza de `c66909d` (tag `v2.8-rifex-prod-pscg-difusion`) a `6f24bab`, promoviendo el trabajo DEV certificado en `origin/develop` @ `b22cf8a` (foundation + SSR auth hardening, plegadas en una sola promoción PROD), autorizado explícitamente por Rodrigo ("GO A PROD"). Nuevo vertical para actividades gratuitas — inscripción pública, QR, scanner, panel, Excel — nunca usa Mercado Pago del organizador ni cobra al participante; vive fuera del onboarding financiero progresivo. Migración PROD (`db/migrations/2026-09-04_inscripciones1_foundation.sql`) aplicada manualmente por Rodrigo vía el SQL Editor de Supabase del proyecto PROD (`wrdkdfuiwlujfxxijpao`) — pre-check y certificación post-apply confirmados limpios (4 tablas, 3 RPCs, RLS habilitado, grants exactos). Deploy de Vercel confirmado `Ready`, alias real a `rifex.pro`. Smoke en vivo: `/inscripciones` con metadata correcta y cero Plus/Gold; las 4 superficies privadas (`/crear-inscripcion` + 3 rutas de `/panel/inscripciones`) devuelven `307` real, idéntico entre navegador/Googlebot/Meta/TikTok, cero fuga de HTML privado. **Prueba funcional completa ejecutada por Rodrigo en PROD real**: actividad de prueba creada sin Mercado Pago, 2 participantes inscritos, QR visible y funcional, check-in validado (PASA), Excel descargado y verificado visualmente con las columnas exactas — consumió el cupo FREE real del mes de esa cuenta, como se esperaba. Regresión completa 802 tests, 801 PASS (mismo flake histórico de XLSX), build limpio. Tag nuevo: `v2.9-rifex-prod-inscripciones-v1`. Detalle completo: `docs/WOP.md`, "RIFEX INSCRIPCIONES V1 FREE + FUTURE BILLING FOUNDATION — PROD PROMOTION (2026-09-04)"; `docs/inscripciones/INSCRIPCIONES_V1_ARCHITECTURE.md`.
>
> **2026-09-04 addendum — RIFEX PSCG + DIFUSIÓN V1.1 MULTIPRODUCTO PROD PROMOTION:** `origin/main`/PROD advances from `7fcc1c5` (tag `v2.7-rifex-prod-progressive-onboarding`), promoting exactly the certified DEV work at `origin/develop` @ `4681770`, spanning two DEV commits (`8ec5787` "PSCG + Difusión V1", `4681770` "Difusión V1.1 Multiproducto"), authorized explicitly by Rodrigo. **PSCG** (Public Surface Classification Guard) is now the transversal rule in PROD: every route is explicitly classified as `PUBLIC_INDEXABLE`, `PUBLIC_NOINDEX`, `PRIVATE_AUTHENTICATED`, or `LEGACY_REMOVED` in a new thin registry (`src/lib/publicSurfaceClassification.js`) that reuses existing infrastructure — no framework, no new table. Real historical debt found during the DEV audit (`/panel/bancos` SSR-hydrates but doesn't redirect; `/trust/verificar`/`/registro/continuar`/`/perfil` have no SSR boundary; the latter two missing from `robots.txt`) is now live on PROD exactly as documented, not fixed by this promotion. **`/difusion`** goes live as a multiproduct guide (classified `PRIVATE_AUTHENTICATED`, boundary `ssr_redirect` — real server-side redirect for anonymous requests, never client-only) covering Rifas ("Precauciones especiales"), Campañas ("Comparte tu causa con claridad"), Eventos ("Guía de difusión"), and Inscripciones ("Próximamente" — `available: false`, no functional copy button, no route/backend). Added only to the authenticated account menu, never to public navigation. All 9 code/test files were wholesale-safe (pre-mission DEV baseline byte-identical to `main`'s current content for the 2 modified files; 7 new files with no PROD divergence possible). An unrelated environment issue (a corrupted `@supabase/supabase-js` install missing its `dist/main/index.js`) was found and fixed before `creationGate.test.mjs` could validate cleanly — confirmed unrelated to any promoted file. 312/312 scoped tests (pscg + difusion + authUxCrawler + publicAudit + publicSurfaceFinalCleanup + blogPrivateProd + creationGate), 738/739 full regression (known XLSX timing flake), clean build, live pre-push smoke clean (307/21-byte anonymous response on `/difusion`, identical across default/Googlebot/Meta/TikTok, zero private-content leakage, Progressive Onboarding gate and API authority both confirmed unaffected). Full detail, exact HEAD, deployment ID, smoke evidence, and new baseline tag: `docs/WOP.md`, "RIFEX PSCG + DIFUSIÓN V1.1 MULTIPRODUCTO — PROD PROMOTION (2026-09-04)".
>
> **2026-09-03 addendum — RIFEX PROGRESSIVE ONBOARDING PROD PROMOTION:** `origin/main`/PROD advances from `37f0820` (tag `v2.6-rifex-prod-public-surface-final`), promoting exactly the certified DEV work at `origin/develop` @ `b996893` ("RIFEX PROGRESSIVE ONBOARDING DEV CERTIFIED"), authorized explicitly by Rodrigo. Closes the one real gap in creator onboarding: `/crear-rifa`, `/crear-colecta`, `/crear-evento` now check real creator eligibility (`assertCreatorEligible`, TRUST-2) server-side, before the form is sent, via a new shared `src/lib/creationGate.js` — not just session presence as before. A non-eligible authenticated user is routed to exactly the existing step that resolves what's missing (`/registro/continuar` for onboarding, `/panel/bancos` for Mercado Pago, `/trust/verificar` structurally for TRUST-3A, which stays dormant), with the original destination preserved via `next` and no open-redirect surface (the destination passed to the gate is always a fixed literal, never `ctx.query`). An eligible user notices zero change — same form, same as before. The 4 code files that needed content (`crear-rifa.jsx`, `crear-colecta.jsx`, `crear-evento.jsx`, `trust/verificar.jsx`) plus the 2 new files (`creationGate.js`, `creationGate.test.mjs`) were wholesale-safe (pre-mission DEV baseline byte-identical to `main`'s current content); `tests/authUxCrawler.test.mjs` needed the mission's exact diff hunk applied by hand on top of `main`'s already-PROD-corrected captcha/RUT assertions, preserved exactly (confirmed with `git apply --check` before applying). The real authoritative protection — `assertCreatorEligible` calls inside `api/rifas`, `api/colectas`, `api/events` — was not part of this mission's file set and remains completely untouched; the new gate is UX-only. 344/344 scoped tests (creationGate + authUxCrawler + publicAudit + publicSurfaceFinalCleanup + blogPrivateProd + trustIdentityGate + sanitizeNextPath + onboardingBancosUx + MP/Trust onboarding suites). Full detail, exact HEAD, deployment ID, smoke evidence, and new baseline tag: `docs/WOP.md`, "RIFEX PROGRESSIVE ONBOARDING — PROD PROMOTION (2026-09-03)".
>
> **2026-09-03 addendum — RIFEX PUBLIC SURFACE FINAL CLEANUP PROD PROMOTION:** `origin/main`/PROD advances from `39b47f5` (tag `v2.5-rifex-prod-auth-crawler`), promoting exactly the certified DEV work at `origin/develop` @ `4a363e7`. Removes an internal legal-review banner ("PENDIENTE DE REVISIÓN POR ABOGADO CHILENO ANTES DE PROD") that was reaching two public pages, `/reglas-iniciativas-premio` and `/terminos-rifas` — no legal review or compliance is declared, the real debt stays tracked in `docs/legal/RIFEX_REVISION_LEGAL_PENDIENTE.txt`, and every substantive term (7% commission, prize delivery, fraud/chargebacks) was verified byte-identical to before. Removes a public placeholder on `/contacto` ("Identidad legal completa del operador: pendiente de confirmación.") without inventing any identity. `/rifas`'s redirect to `/login` is now a real server-side `307` (`getServerSideProps`) instead of client-side-only, same product decision (redirect, not a 410) and same sanitization logic. The "Campañas" navbar item now opens the already-certified `/wizard` campaign explainer (`?modo=colecta`) instead of landing anonymous visitors on an unexplained login wall at `/crear-colecta`. Adds `Organization`+`WebSite` JSON-LD to Home (only verifiable facts) and 4 low-risk security headers to `next.config.mjs` (CSP untouched). All 9 code files were wholesale-safe (pre-mission DEV baseline byte-identical to `main`'s current content); `tests/authUxCrawler.test.mjs` needed one hand-reconstructed test block on top of `main`'s already-PROD-corrected captcha/RUT assertions, preserved exactly. `tests/blogPrivateProd.test.mjs` (PROD-exclusive) re-run and passed in full — Blog privacy unaffected. 196/196 scoped tests, 622/623 full regression (known XLSX timing flake), clean build, self-audit grep zero matches, live multi-UA no-cloaking check MD5-identical. Full detail, exact HEAD, deployment ID, smoke evidence, and new baseline tag: `docs/WOP.md`, "RIFEX PUBLIC SURFACE FINAL CLEANUP — PROD PROMOTION (2026-09-03)".
>
> **2026-09-02 addendum — RIFEX AUTH UX 2026 + CRAWLER CLEANUP PROD PROMOTION:** `origin/main`/PROD advances from `15d7d35` (tag `v2.4-rifex-prod-public-trust`), promoting exactly the certified DEV work at `origin/develop` @ `add98ec`. Navbar: "Crear una iniciativa" removed, "Iniciar sesión" renamed to "Ingresar" (desktop + mobile), central menu now grid-centered against the true viewport instead of just free space. Login/Register: rebuilt around a new shared, presentation-only `AuthShell` component (title "Ingresar" / "Crear cuenta", footer cross-links), with zero Auth-logic changes — `main`'s own real inline hCaptcha (`window.hcaptcha` + `/api/verify-captcha`) and unconditional RUT requirement were preserved exactly; the D5-FINAL `captchaGate.js` DEV bypass was never brought in. Real server-side auth boundary added via `getServerSideProps` + `getSupabaseServer` on the 5 previously client-only-gated surfaces (`/crear-rifa`, `/crear-colecta`, `/crear-evento`, `/panel`, `/mis-iniciativas`) — closes a real content leak where full form/dashboard HTML was reaching anonymous requests before a client `useEffect` redirect fired; anonymous requests now get a real HTTP 307 to `/login?next=<path>`. Blog copy neutralized ("cerraron su rifa" → "organizadores de nuestra comunidad"); Blog itself remains private/noindex, unchanged. 12 files taken wholesale from `develop` (pre-mission baseline confirmed byte-identical to `main` first); `login.jsx`/`register.jsx`/`blog/index.js` manually reconstructed against `main`'s real content to preserve PROD-only captcha/RUT logic. 142/142 scoped tests, 582/583 full regression (known XLSX timing flake), clean build, self-audit grep zero matches. Full detail, exact HEAD, deployment ID, smoke evidence, and new baseline tag: `docs/WOP.md`, "RIFEX AUTH UX 2026 + CRAWLER SURFACE CLEANUP — PROD PROMOTION (2026-09-02)".
>
> **2026-09-02 addendum — RIFEX V4 PUBLIC TRUST (A1-A7) + STAGE 2 PROD PROMOTION:** `origin/main`/PROD advanced from `a2d6a60` via a file-exact, mechanically-scoped promotion of 61 code files + targeted addenda to `docs/WOP.md`/`docs/CURRENT_STATE.md`/`docs/handover/NUEVA_SESION_PROMPT.md`. Pre-flight found that PROD had never received the V4 Public Trust foundation (A1-A7) that Stage 2 depends on — `cookies.js`, `privacidad.js`, `uso-aceptable.js`, `politica-eventos.js`, `politica-campanas.js`, `reembolsos.js`, `reglas-iniciativas-premio.js`, `terminos-rifas.js`, `publicMetadata.js`, `robots.txt`, `sitemap.xml` simply did not exist on `main`; Rodrigo authorized expanding the release scope after this was demonstrated. Scope was computed mechanically as the union of the V4 A1-A7 and Stage 2 commit-range diffs on `origin/develop`, with 11 files explicitly excluded (DEV-only `DevBanner.jsx`/`captchaGate.js`/`_app.js` wiring, Blog files already promoted separately, and 4 files exclusive to `main`'s own release history) — all 61 promoted files verified byte-identical to their `develop` source before commit. Content self-audit: zero real webhook/Payment Engine/feePolicy/providerRegistry/commission/Argentina/migration references in the code diff. PROD now serves: the public `/rifas` catalog removed, canonical always `rifex.pro`, `robots.txt`/`sitemap.xml` live for the first time, `noindex` on every authenticated page, `/terminos` as a corporate-only document, `/wizard` representing exclusively Eventos + Campañas, navbar reduced to Eventos/Campañas/Cómo funciona, footer with "Comisión" and the community-discovery line. Rifas as a product (creation, panel, `/mis-iniciativas`, ownership/payment/draw logic) fully preserved — only its public catalog exposure and public wording changed. Full detail, exact HEAD, deployment ID, smoke results and new baseline tag: `docs/WOP.md`, "RIFEX V4 PUBLIC TRUST (A1-A7) + STAGE 2 (PUBLIC IDENTITY + POLICIES) — PROD PROMOTION (2026-09-02)".
>
> **2026-09-01 addendum — RIFEX BLOG PRIVATE PROD PROMOTION, `main`/PROD now at `2c39587`, tagged `v2.3-rifex-prod-blog-private`:** every note below naming `main` as `5145d91` or earlier is historical, not current. `origin/main` advanced `b9a9798` → `2c39587` (clean fast-forward). This promotes exactly one thing: the Blog feature (footer link, 4 pages, 2 read APIs) becomes auth-gated on PROD — no V4 Public Trust infrastructure (`robots.txt`, `sitemap.xml`, canonical/OG metadata), no Identity/Policies work, and no payment/webhook/Trust/Events/Campañas/commission logic moved. Since `develop`'s Blog Private commit (`bf5745b`) was built on top of V4 A+B changes `main` never received, the equivalent patch was manually rebuilt against `main`'s real files rather than cherry-picked; `Layout.jsx`'s change was reduced to a single footer-line removal. Target state confirmed live: footer/navbar/authenticated-menu all without a Blog link, `noindex, nofollow, noarchive` on all 4 pages, both read APIs return `401 missing_auth` anonymously, anonymous direct access redirects to `/login`, authenticated access unaffected, nothing deleted. 14/14 scoped tests + 440/441 full regression (1 pre-existing unrelated XLSX timing flake) + clean build; scope self-audit confirmed exactly 8 changed files with zero payment/webhook/Trust/commission references anywhere in the diff. Deployed via the existing GitHub → Vercel auto-deploy integration, verified via real HTTP smoke against `rifex.pro`. Full detail: `docs/WOP.md`, "RIFEX BLOG PRIVATE — PROD PROMOTION (2026-09-01)", and `docs/releases/RIFEX_BLOG_PRIVATE_PROD_PROMOTION_2026-09-01.md`. Per Rodrigo's authorization, PROD access ends here — ETAPA 2 (Identidad Pública + Políticas) proceeds on `develop` only, autonomously, no further PROD writes.
>
> **2026-08-31 addendum — RIFEX CONTROLLED PROD RELEASE, `main`/PROD now at `5145d91`:** every note below naming `main` as `3f3d6c4`, `5c15624`, or `c944bb3` is historical, not current. `origin/main` advanced `e7311c1` → `5145d91` (clean fast-forward, verified via real Git state immediately before and after the push) via a strictly controlled, file-exact promotion of exactly 4 certified `develop` blocks: **Cumplimiento V1** (C1/C3/C4/C5 — post-raffle compliance case tracking, foundation/communications/timeline-escalation/admin-review), **Onboarding + Bancos/MP** (neutral onboarding, `/panel/bancos` state machine, server-side MP revalidation, Stripe visual-only), **Events Capacity + Live Attendance** (EVENT-8 — `events.capacity` aforo invariant, scanner live attendance derived solely from `event_tickets.used_at`), and **Home V1** (public Home redesigned around Eventos/Entradas/Campañas, real photo hero, premium capability cards). All 5 required PROD migrations applied with per-migration drift re-checks and postchecks; financial table row counts unchanged (`raffles`=7, `tickets`=420, `payments`=4); 65-file autoaudit against the pre-release baseline confirmed no leakage beyond the 4 certified blocks. Deployed via the existing GitHub → Vercel auto-deploy integration, aliased to `rifex.pro`, verified via real HTTP content (not deployment-status trust alone). Excluded from this release, remaining on `develop` only: Difundir iniciativa, TXT V4, MP QUALITY 100 work, new policies, Argentina reactivation, Stripe real integration. Full detail: `docs/WOP.md`, "RIFEX CONTROLLED PROD RELEASE (2026-08-31)", and `docs/releases/RIFEX_CONTROLLED_PROD_RELEASE_2026-08-31.md`.
>
