...
>
> **2026-08-26 addendum — TRUST-1 (onboarding universal) code-complete, DEV migration NOT applied:** implemented `trust_onboarding` (new table, RLS default-deny total, no client access at all — stricter than `users_profile`), `src/lib/trustOnboardingPolicy.js`/`trustOnboardingGate.js`, `GET/POST /api/onboarding/trust/*`, `/registro/continuar`, and a server-side gate wired into 13 real sensitive endpoints across Rifas/Colectas/Eventos. 29 real tests pass, including an adversarial test proving the client can never smuggle `onboarding_completed_at` through the API. Full regression clean. Migration `db/migrations/2026-08-26e_trust1_onboarding.sql` written, reviewed, **not applied** — blocked on Rodrigo's separate, explicit authorizations for applying it in `rifex-dev`, creating fixtures if needed, pushing to `origin/develop`, and deploying DEV. Deploying the code without the migration would break every creation/publish/admin action for everyone (fails closed by design) — they must ship together. **TRUST-2 onward remains design-only, not authorized.**
>
