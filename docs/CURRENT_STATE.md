...
>
> **2026-08-25 addendum (updated):** EVENT-4 (Staff + Scanner + Check-in) is **DONE** — implemented, migrated onto `rifex-dev`, and certified with 36/36 automated tests (including 20-concurrent exactly-once check-in). Full specification at `docs/events/EVENT4_STAFF_SCANNER_CHECKIN.md` (canonical). Live-schema verification of `rifex-dev` was completed functionally (RLS/RPC/table checks), not via a raw catalog dump — the Supabase CLI's `db push`/`db pull` remain unusable for this project until someone authorizes a migration-history repair (see `docs/WOP.md`, Risks/pending item 9). The `rifex-dev` database password **still needs rotation** — deferred to a following session by explicit user decision, not done yet; never reuse the credential that was exposed by a `--dry-run` dump earlier on 2026-08-25. The scanner's camera/visual UI was **not** verified in a real browser this session (tooling limitation, not a code issue) — recommend a real-device test before relying on it. See `docs/WOP.md`, Risks/pending items 8–11, and "EVENT-4 checkpoint", for full detail. NEXT is EVENT-5, not scoped, not authorized.
