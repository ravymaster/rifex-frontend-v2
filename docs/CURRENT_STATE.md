...
>
> **Separate, later 2026-08-25 addendum — PRE-LAUNCH-FIX-3, RESOLVED:** a real Supabase Security Advisor alert (`rls_disabled_in_public`, CRITICAL) for `public.raffle_date_extensions` — unrelated to Events, an omission from the original DRAW-1 migration — was found, demonstrated (an anonymous key could `INSERT` into it with zero error), and fixed in **both** `rifex-dev` and PROD (`wrdkdfuiwlujfxxijpao`) with a single versioned `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` (`db/migrations/2026-08-25c_prelaunch_fix3_raffle_date_extensions_rls.sql`), identical to the already-certified `legal_declarations` pattern. PROD's Security Advisor now reports zero ERROR-level issues. See `docs/WOP.md`, "PRE-LAUNCH-FIX-3", for full detail including a separately-flagged, unrelated finding (the `rifex.pro` custom domain's TLS handshake fails from this session's network, while the underlying Vercel deployment itself is healthy).
