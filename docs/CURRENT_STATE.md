...
> **2026-09-03 addendum (most recent) — RIFEX PROGRESSIVE ONBOARDING, `origin/develop` avanza desde `4a363e7`, DEV only, misión autónoma:** implementa el gate progresivo de elegibilidad al intentar CREAR una Rifa/Campaña/Evento — registro/login/navegación de la comunidad ya no requerían habilitación completa (eso ya funcionaba), pero los 3 formularios de creación (`crear-rifa.jsx`, `crear-colecta.jsx`, `crear-evento.jsx`) se rendereaban completos para cualquier autenticado sin importar si ya era elegible; el único guardia era un `useEffect` cliente-side fail-open que corría después de que el formulario ya había montado. Auditoría confirmó que Rifex ya tenía todas las piezas necesarias: `assertCreatorEligible` (TRUST-2, única autoridad real de elegibilidad, fail-closed, `mp_identity_match==='matched'` como único estado válido de MP) y `sanitizeNextPath` (único saneador de redirect interno del repo, ya reusado en 5+ puntos) — cero piezas nuevas de lógica de negocio. Fix: nuevo `src/lib/creationGate.js` (`resolveCreationGate(ctx, destinationPath)`), llamado desde `getServerSideProps` de los 3 formularios con un literal fijo (nunca `ctx.query` — sin superficie de open-redirect), que verifica sesión y luego `assertCreatorEligible`, mapeando cada `reason` real al paso existente que lo resuelve (`onboarding_incomplete`/`identity_incomplete` → `/registro/continuar`; `mp_not_connected`/`mp_identity_mismatch`/`mp_check_pending` → `/panel/bancos`; `identity_verification_required` → `/trust/verificar`, mapeo estructural — TRUST-3A sigue inalcanzable por política, `isIdentityVerificationRequiredForCreators()` en `false`, no se activó). Cubre todos los entry points sin tocar ningún botón/CTA: `getServerSideProps` de Next.js corre en cada request, incluida navegación client-side, por lo que gatear las 3 páginas destino basta. La protección autoritativa real (`assertCreatorEligible` dentro de `api/rifas`, `api/colectas`, `api/events`) queda sin tocar — confirmado por diff vacío en `src/pages/api/`. `/trust/verificar` recibió plomería mínima de `next` (mismo `sanitizeNextPath`) y un botón "Continuar" para cuando esa rama se active en el futuro. 27 tests nuevos en `creationGate.test.mjs` (funcionales reales, Supabase mockeado vía prototype-patch) cubriendo los 20 escenarios requeridos; `authUxCrawler.test.mjs` actualizado (no debilitado) para reflejar que 3 de las 5 páginas SSR-boundary ahora pasan por el gate compartido. Validación: 268/268 tests específicos, regresión completa 635/636 (mismo flake XLSX histórico), build limpio, self-audit sin coincidencias reales, smoke test en vivo confirmando 307 reales sin fuga de formulario. Auto-auditoría adversarial post-implementación (skip onboarding, URL directa, forjar `next` externo, loop, degradar `unknown`→`matched`, crear vía API sin elegibilidad, perder destino en OAuth de MP) — ningún fallo encontrado. `origin/main` (PROD) no referenciado por esta rama. Detalle completo: `docs/WOP.md`, "RIFEX PROGRESSIVE ONBOARDING — CREACIÓN DE INICIATIVAS (2026-09-03) — DEV only, autonomous mission". **Próximo paso: eventual promoción controlada a PROD, sujeta a autorización explícita — todavía no iniciada.**
>
> **2026-09-03 addendum (follow-up) — blocker `/terminos-rifas` resuelto:** Rodrigo autorizó explícitamente retirar también de `/terminos-rifas` el banner interno de revisión legal pendiente (opción (b) de la decisión humana). Ejecutado eliminando únicamente el párrafo del banner — sin declarar revisión ni cumplimiento jurídico, sin tocar ninguna condición sustantiva (comisión 7%, entrega, fraude/chargebacks verificados intactos), deuda real sigue trackeada en `docs/legal/`. Los dos tests certificados que exigían el banner se actualizaron para exigir su ausencia. Re-validado: suite específica 40/40, authUxCrawler+publicAudit 182/182, regresión completa 608/609 (mismo flake XLSX), build limpio, self-audit limpio, re-auditoría directa de ambas páginas confirmando ausencia del banner. Ya no queda ningún blocker abierto de esta misión. Detalle: `docs/WOP.md`, sección "Blocker resuelto" dentro de la entrada PUBLIC SURFACE FINAL CLEANUP.
>
