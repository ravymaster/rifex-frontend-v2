...
**Riesgo residual documentado, no resuelto en este sprint**: ~15s de cómputo puro (sin contar las 4 queries reales a Supabase que preceden) cabe cómodamente en el timeout por defecto de Vercel Pro (60s) pero **excedería el límite de 10s de Vercel Hobby**. No se verificó en este sprint qué plan de Vercel usan `rifex-frontend-main`/`rifex-frontend-v2` — recomendado confirmar antes de considerar el límite de 20.000 filas "seguro en producción" bajo carga real. No se implementó ExcelJS streaming: la medición real (~15s) no lo justificó todavía: streaming es un cambio de arquitectura no trivial que se evaluará solo si esta cifra deja de ser segura.
