...
**PROD functional health, separately confirmed**: the live Vercel deployment behind `rifex-frontend-v2` responds `200` on `/api/rifas` when hit directly (`https://rifex-frontend-v2-cfvr2oh8j-rifex.vercel.app`, the current Production-target deployment) — app and DB both healthy after the fix. **Unrelated finding, flagged but not acted on**: the custom domain `rifex.pro` itself fails its TLS handshake from this session's network (`tlsv1 alert internal_error`, a server-side TLS alert — confirmed not a local/hotel-network issue, since `github.com` and the direct `*.vercel.app` deployment URL both connect fine over the same connection). Pre-existing, unrelated to this RLS fix (which is database-only and has no relationship to Vercel's domain/TLS layer) — out of scope for this incident, needs separate investigation (likely a `rifex.pro` custom-domain cert/edge config issue on Vercel's side).
