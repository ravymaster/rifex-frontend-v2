...
`origin/main`/PROD avanza de `3a6517b` a `2e2f905` (tag `v3.4-rifex-prod-eventos-portada-cta`), vía worktree aislado `hotfix/eventos-portada-cta-2026-09-14` construido directo desde `origin/main` (sin tocar `develop`). Corrige dos fallas reportadas en `/crear-evento` y en la página pública de un evento.

**Causa raíz 1 — "Bucket not found" al subir portada**: el bucket de Storage `event-photos` (`src/pages/api/events/upload-photo.js`) nunca existió en el proyecto Supabase de PROD (`wrdkdfuiwlujfxxijpao`) — se creó a mano solo en `rifex-dev` durante EVENT-1 (Fase 14) y esa creación nunca se replicó a PROD, porque ningún bucket de este repo se había creado vía migración SQL hasta ahora (`raffle-prizes`/`colecta-photos`/`avatars` tampoco tienen migración de bucket propia). Confirmado empíricamente vía `GET /storage/v1/bucket` en ambos proyectos antes de escribir la corrección. Migración versionada (`db/migrations/2026-09-14_hotfix_event_photos_bucket.sql`, idempotente) creada como registro del estado esperado y aplicada también a `rifex-dev` (no-op, ya lo tenía); el bucket en PROD se creó vía la API de Storage de Supabase (`POST /storage/v1/bucket`, usando `service_role`) al no contar localmente con credenciales de conexión Postgres directa a PROD — mismo resultado final que el `insert into storage.buckets` de la migración. Mismo modelo de privilegio mínimo que `colecta-photos`/`raffle-prizes`/`avatars`: público de lectura (la portada se sirve sin sesión), sin ninguna policy de `storage.objects` (toda escritura pasa exclusivamente por `upload-photo.js`, que exige Bearer auth y usa `service_role`), rutas de objeto no predecibles (`userId/timestamp-randomUUID-nombre.jpg`, `upsert:false`). Además: los errores del driver de Storage ya no se reenvían crudos al cliente (antes filtraba mensajes como "Bucket not found"); se agregó una guardia de dimensiones absurdas antes de dibujar en canvas (defensa contra bombas de descompresión); y si la subida falla, ahora se limpia la previsualización local (antes quedaba visible una "foto" que en realidad nunca se había guardado, la causa exacta de la confusión reportada).

**Causa raíz 2 — botón "Comprar entradas" gris/no comunica que es pulsable**: la clase `.ctaBtn` (`src/styles/evento.module.css`) tenía `background`/`color`/`cursor` grises fijos en la regla base, sin condicionar al atributo `disabled` real — indistinguible entre "listo para comprar" y "realmente deshabilitado durante la compra". La lógica de validación (`handleBuy` en `eventos/[id].jsx`) ya era correcta: rechazaba selección vacía y email inválido antes de llamar al checkout, nunca creaba una preferencia con datos inválidos. Se reemplazó el CSS por el mismo patrón ya certificado y en producción en Colectas (`.submitBtn`): degradé de marca (`var(--ultramar)`→`var(--trebol)`) + blanco cuando es pulsable, `opacity:.65` solo cuando `disabled=true`. Se agregó además una guardia síncrona anti-doble-toque (`if (buying) return;` al inicio de `handleBuy`), `trim()` del nombre del comprador antes de decidir si va vacío, y `aria-busy` en el botón.

**Payment Engine/checkout/webhook/OAuth MP/comisión 7%/Medidor QR/Admin Analytics/puerta DEV/Campañas/Rifas/Inscripciones intactos**: certificado en `tests/hotfixEventosPortadaCta.test.mjs` (27 escenarios) vía comparación byte-a-byte contra `origin/main` de 20 archivos protegidos (checkout/webhook/OAuth MP/reconciliación, Medidor QR, Campañas/Rifas/Inscripciones) más confirmación de que Admin Analytics/puerta DEV no existen en este hotfix. `checkout.js` (creación de preferencia de pago del propio módulo Eventos) permanece byte-a-byte idéntico — el ajuste fue enteramente client-side.

**Validación**: 27/27 tests específicos verdes; regresión completa 1257/1274 pass — 3 fallas, las 3 confirmadas preexistentes y ajenas a este hotfix (reproducidas idénticas en un checkout limpio de `origin/main` sin ningún cambio mío): el flake histórico conocido de timing en `eventAnalyticsWorkbook.test.mjs`, y dos aserciones de `tests/humanUrlStandard2026.test.mjs` que comparan contra `origin/develop` (un objetivo que se mueve — avanzó por la misión Admin Analytics, ajena a este hotfix); build limpio; `git diff --check` limpio; diff acotado a 6 archivos (4 modificados + 1 migración + 1 test nuevo).

**Smoke PROD en vivo**: deploy Vercel confirmado `Ready` (Production) tras el push. Ciclo escritura→lectura→borrado de un archivo de prueba contra el bucket real `event-photos` de PROD, verificado también vía listado autenticado que no queda rastro. CSS del degradé de marca confirmado servido en la página real de un evento publicado (`rifex.pro/eventos/rifex-events-financial-cert-46387a`). `rifex.pro` responde `200` en landing y `/eventos` antes y después del despliegue.

**RIFEX EVENTOS PORTADA + CTA HOTFIX: PROD CERTIFIED.**
...
**Divergencia real de dos puntos (archivos, no commits — `develop` acumula 100+ commits de módulos ya promovidos en ciclos anteriores o deliberadamente fuera de esta promoción)**: 70 archivos difieren; **39 promovidos**, el resto excluido.

**Promovido**: 2 migraciones aditivas (`2026-09-08_medidor_qr_v1.sql` — dominio nuevo e independiente, tablas `medidores_qr`/`medidor_qr_responses`/`medidor_qr_visits`/`medidor_qr_destination_clicks`/`medidor_qr_free_usage`, RLS propia, cero referencia a `raffles`/`events`/`registration_*`/pagos, confirmado por grep de `create table`/`alter table`; `2026-09-09_medidor_qr_free_quota_10.sql` — sube el cupo de 1 a 10/mes vía `pg_advisory_xact_lock` + conteo, reemplaza el `UNIQUE` anterior); toda la superficie de Medidor QR (10 endpoints API, 6 páginas, 4 libs, shell público sin Layout global); integración necesaria en Navbar/Footer (`Layout.jsx`), Mis Iniciativas, Home (card + PSCG + robots.txt + sitemap.xml); Home Hero 2026 completo (`index.js`, `index.module.css`, 4 assets nuevos, retiro de `rifex-hero-events.png`); `package.json` (script `test:medidor-qr`); doc nueva `docs/medidor-qr/MEDIDOR_QR_V1.md`; 3 archivos de test (`homeHero2026.test.mjs`, `medidorQr.test.mjs` nuevos; `humanSlugV2.test.mjs` — fix necesario de un test existente que contaba migraciones por prefijo de fecha, ahora excluye explícitamente la nueva migración de Medidor QR por nombre para no aflojar el invariante que protege).

**Excluido, con motivo verificado (no una lista genérica)**:
- `db/migrations/2026-09-07_raffle_slug_features.sql`: **`main` tiene la versión más avanzada** — ya incluye la corrección de `requires_transfer_procedures`/`transfer_expenses_owner`/`transfer_conditions` en `create_raffle_with_declarations` que la copia de `develop` no tiene. Copiarla habría sido una regresión real sobre Rifas.
- `src/components/DevBanner.jsx`, `src/lib/captchaGate.js`, cambios en `_app.js`/`login.jsx`/`register.jsx`: tooling exclusivo DEV. `main` ya certifica su ausencia con un test propio (`authUxCrawler.test.mjs`: `assert.doesNotMatch(login, /captchaGate/)` + `assert.ok(!fs.existsSync(...'captchaGate.js'))`) — se preservó la versión de `main` de ese test, no se copió la de `develop` (que asegura lo contrario, correcto para DEV).
- `src/components/rifex/BuyerForm.jsx`, `RaffleIntroModal.jsx`: cambios de Rifas ajenos a esta misión. **Hallazgo colateral, no corregido acá** (fuera de alcance, requiere autorización propia): `BuyerForm.jsx` en `main` destructura la prop `selected=[]`, pero su único invocador (`rifas/[id].jsx`) le pasa `quantity={quantity}` — el modal nunca calcula bien cantidad/total si llega a montarse. `RaffleIntroModal.jsx` no tiene ningún import activo en `main` (código huérfano). Reportado aparte a Rodrigo, no promovido junto con Medidor QR/Home Hero.
- `src/pages/api/blog/*`, `src/pages/blog/index.js`, `tests/authUxCrawler.test.mjs`, `tests/blogPrivateProd.test.mjs`: reorden de auth/imports de Blog y test guard ajenos, no relacionados con esta misión.
- `docs/difusion/DIFUSION_V1.md`, `docs/inscripciones/*` (4 archivos), `docs/trust/PROGRESSIVE_ONBOARDING_GATE.md`, `docs/public-surface/PUBLIC_SURFACE_CLASSIFICATION_GUARD.md`, `docs/public-surface/FINAL_PUBLIC_SURFACE_CLOSURE.md`, `docs/public-surface/PRODUCT_LANDINGS_V1.md`: deriva documental preexistente de otros módulos, sin relación con Medidor QR/Home Hero, cero impacto funcional si quedan fuera.
- `docs/releases/*` (4 archivos) y `db/migrations/2026-08-26d5_ar2_country_columns_reconstructed.sql`: existen solo en `main` (nunca se mergea la historia de `develop`); no se tocan ni se borran.
- `docs/WOP.md`/`docs/CURRENT_STATE.md`/`docs/handover/NUEVA_SESION_PROMPT.md`: no se copia la versión de `develop` (arrastra el changelog completo de misiones DEV-only ajenas) — se edita la versión propia de `main` con esta entrada, mismo patrón que promociones anteriores.

**Payment Engine/checkout/webhook/comisión intactos**: `git diff origin/main -- src/pages/api/checkout/mp.js src/pages/api/checkout/colecta.js src/pages/api/checkout/webhook.js` = 0 líneas; `src/lib/paymentEngine/` = 0 líneas; `src/lib/feePolicy.js` = 0 líneas. Rifas/Eventos/Campañas/Inscripciones/RLS existente no tocados por ninguna de las 2 migraciones (ambas dominio Medidor QR puro).
...
**Classification result — all 9 code/test files WHOLESALE-SAFE**: `src/components/Layout.jsx` and `public/robots.txt` confirmed byte-identical between PROD's current content and DEV's pre-mission baseline (`b996893`, itself already PROD's baseline from the prior Progressive Onboarding promotion) — checked out `4681770`'s version of each. `src/lib/publicSurfaceClassification.js`, `tests/pscg.test.mjs`, `src/pages/difusion.jsx`, `src/lib/difusionGuides.js`, `tests/difusion.test.mjs`, `docs/public-surface/PUBLIC_SURFACE_CLASSIFICATION_GUARD.md`, `docs/difusion/DIFUSION_V1.md` are new files with no PROD divergence possible — checked out wholesale from `4681770`. Verified the resulting diffs against PROD for `Layout.jsx` (adds the `noarchive` prop + the "Difusión" menu item, nothing else) and `robots.txt` (adds exactly one `Disallow: /difusion` line) — no PROD-specific content lost.

**DOC/TEST reconstructed as PROD-specific promotion entries**: `docs/WOP.md`, `docs/CURRENT_STATE.md`, `docs/handover/NUEVA_SESION_PROMPT.md` (this entry and its siblings), rather than copying DEV's develop-branch-history-relative prose.

**DO-NOT-PROMOTE**: none. Nothing in this delta touches Payment Engine, Mercado Pago, `marketplace_fee`, commission, webhooks, Supabase schema, RLS, migrations, Trust backend, `assertCreatorEligible`, Progressive Onboarding, `creationGate`, Events/Campaign/Raffle business logic, Cumplimiento, Blog functionality, login/register logic, captcha, RUT, Argentina, C6, or any Inscripciones backend.

**PROD divergences preserved, verified explicitly**: `login.jsx`/`register.jsx` real inline hCaptcha + unconditional RUT — untouched, reconfirmed live (`window.hcaptcha`/`h-captcha` present on `/login`, `captchaGate.js` confirmed absent from the candidate). `tests/blogPrivateProd.test.mjs` (PROD-exclusive) run against the candidate and passed in full. `tests/creationGate.test.mjs` (Progressive Onboarding, PROD-exclusive scope for this promotion) run against the candidate and passed in full — confirming the creator-eligibility gate on `/crear-rifa`/`/crear-colecta`/`/crear-evento` is completely unaffected by this promotion. `assertCreatorEligible`, every API route (`api/rifas`, `api/colectas`, `api/events`), and `src/lib/creationGate.js` confirmed byte-for-byte unchanged via `git diff --stat` against the candidate (empty output).

**Delta actually promoted, matching the certified DEV missions**:
- **PSCG**: new `src/lib/publicSurfaceClassification.js` — `PSCG_CATEGORY` (4 categories: `PUBLIC_INDEXABLE`, `PUBLIC_NOINDEX`, `PRIVATE_AUTHENTICATED`, `LEGACY_REMOVED`), `PSCG_BOUNDARY` (5 real auth-boundary subtypes found during audit), `PSCG_REGISTRY` (17 `PUBLIC_INDEXABLE` + 4 `PUBLIC_NOINDEX` + 11 `PRIVATE_AUTHENTICATED` incl. `/difusion` + 1 `LEGACY_REMOVED`). No framework, no new table, no build-time enforcement — a thin registry over existing infrastructure. `Layout.jsx` gains an optional `noarchive` prop (default `false`, byte-identical output for every existing caller).
- **Difusión V1.1 (multiproduct, the final DEV-certified state)**: `/difusion`, classified `PRIVATE_AUTHENTICATED` with boundary `ssr_redirect` — `getServerSideProps` redirects anonymous requests directly to `/login?next=/difusion`, same pattern as `mis-iniciativas.jsx`. New `src/lib/difusionGuides.js` (pure data, no JSX/network/AI) with content for 4 products: Rifas ("Precauciones especiales"), Campañas ("Comparte tu causa con claridad"), Eventos ("Guía de difusión"), Inscripciones ("Próximamente", `available: false`, no functional copy button, no route/backend). Selector is a `role="tablist"` segmented control, `useState`-driven, default "Eventos". "Difusión" added only to `Layout.jsx`'s `accountItems` (authenticated menu), never to public nav/footer/Home. `robots.txt` gains `Disallow: /difusion`; absent from `sitemap.xml`.

**Validation on the exact PROD candidate**: an unrelated environment issue was found and fixed before validation could complete — `node_modules/@supabase/supabase-js`'s `dist/main/index.js` was missing from the fresh `npm install` (broken/incomplete download, unrelated to any promoted file), causing `tests/creationGate.test.mjs` to fail at module-resolution time; reinstalling the package (`npm install @supabase/supabase-js@2.57.2 --no-save`) restored it, confirmed via a clean 27/27 re-run. `pscg.test.mjs` + `difusion.test.mjs` + `authUxCrawler.test.mjs` + `publicAudit.test.mjs` + `publicSurfaceFinalCleanup.test.mjs` + `blogPrivateProd.test.mjs` (PROD-exclusive) + `creationGate.test.mjs` (PROD-exclusive) → **312/312 PASS**. Full regression `node --test 'tests/*.test.mjs'` → **738/739** (the same pre-existing `eventAnalyticsWorkbook.test.mjs:93` XLSX `writeBuffer` timing flake, identical signature — ~67.1s against a 20s budget at maximum simulated load under the fresh-install machine load, reproduced, not a new failure). `npm run build` → clean, zero errors; `/difusion` confirmed `ƒ` (dynamic/server-rendered) in the manifest.

**Self-audit**: grep across the full candidate diff for `User-Agent|Googlebot|facebookexternalhit|TikTokBot|bypass|dangerously|payment|webhook|marketplace_fee|argentina|service_role|migration|Trust write|openai|warp|gpt|oauth` → zero real matches. `git diff --stat origin/main -- src/pages/api/ src/lib/trustIdentityGate.js src/lib/creationGate.js` → empty (all three layers untouched).

**Pre-push live smoke on the candidate** (local `next start` on the exact promoted code, port 3055): anonymous `GET /difusion` → real `307` to `/login?next=/difusion`, body **21 bytes**, zero occurrences of `rifa|sorteo|premio|campaña|evento|TikTok|Facebook|"ejemplo de publicaci"` in the response body. Multi-UA check identical across default/Googlebot/Meta/TikTok (307/21 bytes each) — zero cloaking. `sitemap.xml` served live: zero `difusion` occurrences. `robots.txt` served live: `Disallow: /difusion` present. Home's JSON-LD confirmed to contain zero `difusion` references. Regression spot-check on the same running candidate: `/crear-rifa`, `/crear-colecta`, `/crear-evento`, `/mis-iniciativas` all real `307`s (Progressive Onboarding gate intact, anonymous); `/blog`, `/login`, `/register`, `/`, `/eventos` all `200`; `POST /api/rifas` without auth → `401` (API authority independent and intact); `Layout.jsx`'s `accountItems` confirmed to list "Difusión" between "Mis iniciativas" and "Bancos & Pagos".
...
- **WHOLESALE-SAFE (6 files)**: `src/pages/crear-rifa.jsx`, `src/pages/crear-colecta.jsx`, `src/pages/crear-evento.jsx`, `src/pages/trust/verificar.jsx` — each confirmed byte-identical between PROD's current content and DEV's pre-mission baseline (`4a363e7`), since that baseline is itself the exact commit the prior PUBLIC SURFACE FINAL CLEANUP promotion already brought to PROD. Checked out `b996893`'s version of each wholesale. `src/lib/creationGate.js` and `tests/creationGate.test.mjs` are new files with no PROD divergence possible — checked out wholesale.
- **RECONSTRUCT (1 file)**: `tests/authUxCrawler.test.mjs` — `main`'s current version diverges from `develop`'s pre-mission baseline by the same 3 known, correct PROD-specific captcha/RUT corrections established across every prior promotion (real inline hCaptcha assertions instead of `verifyCaptchaOrDevBypass`/`captchaGate`, unconditional-RUT assertion, `captchaGate.js`-must-not-exist assertion). This mission's actual change to this file was a single, self-contained diff hunk (splitting `PROTECTED_PAGES` into `DIRECT_BOUNDARY_PAGES` + `GATED_CREATION_PAGES`, entirely below and independent of the captcha/RUT test block). `git apply --check` confirmed the exact mission diff applies cleanly onto `main`'s real file with zero conflict; applied directly rather than hand-retyped, then diffed the result against `develop`'s `b996893` version to confirm the only remaining differences are precisely the 3 known, correct PROD divergences.
- **DOC/TEST**: `docs/WOP.md`, `docs/CURRENT_STATE.md`, `docs/handover/NUEVA_SESION_PROMPT.md` reconstructed as PROD-specific promotion entries (this entry and its siblings) rather than copying DEV's develop-branch-history-relative prose; `docs/trust/PROGRESSIVE_ONBOARDING_GATE.md` promoted wholesale (describes the architecture/mechanism, not develop-branch-relative history).
- **DO-NOT-PROMOTE**: none. Nothing in this mission's DEV delta touched Payment Engine, Trust backend, webhooks, commission, migrations, or Argentina.

**PROD divergences preserved, verified explicitly**:
- `login.jsx`/`register.jsx` real inline hCaptcha + unconditional RUT — untouched by this mission's file set (neither file was part of the DEV delta), reconfirmed present via `authUxCrawler.test.mjs` and `tests/blogPrivateProd.test.mjs` passing on the candidate.
- `tests/blogPrivateProd.test.mjs` (PROD-exclusive, does not exist on `develop`) — run against the candidate and passed in full.
- `assertCreatorEligible` (`src/lib/trustIdentityGate.js`) and every API route (`api/rifas`, `api/colectas`, `api/events`) — not part of this mission's file set, untouched; confirmed via `git diff --stat` against the candidate showing zero changes under `src/pages/api/` or `src/lib/trustIdentityGate.js`.

**Delta actually promoted, matching the certified DEV mission**:
- New `src/lib/creationGate.js` (`resolveCreationGate(ctx, destinationPath)`): server-side gate called from the `getServerSideProps` of the 3 creation pages, checking session then `assertCreatorEligible`, mapping each real failure `reason` to the existing step that resolves it (`onboarding_incomplete`/`identity_incomplete` → `/registro/continuar`; `mp_not_connected`/`mp_identity_mismatch`/`mp_check_pending` → `/panel/bancos`; `identity_verification_required` → `/trust/verificar`, structural only — TRUST-3A remains dormant). `destinationPath` is always a fixed literal the page passes, never `ctx.query`.
- `crear-rifa.jsx`/`crear-colecta.jsx`/`crear-evento.jsx`: `getServerSideProps` now calls `resolveCreationGate`; the prior client-side `useEffect` + `resolveTrustOnboardingRedirect` fail-open check removed as dead weight now that the real boundary is server-side and runs before any markup is sent.
- `trust/verificar.jsx`: minimal `next` preservation (`sanitizeNextPath`) and a "Continuar" button on the `approved` branch, for completeness if TRUST-3A is ever activated by a future decision; also hardened the session-check `useEffect` to guard on `router.isReady` and preserve the real requested path on the login redirect.

**Validation on the exact PROD candidate**: `creationGate.test.mjs` + `authUxCrawler.test.mjs` + `publicAudit.test.mjs` + `publicSurfaceFinalCleanup.test.mjs` + `tests/blogPrivateProd.test.mjs` (PROD-exclusive) + `trustIdentityGate.test.mjs` + `sanitizeNextPath.test.mjs` + `onboardingBancosUx.test.mjs` + `mpIdentityMatchGate.test.mjs` + `mpIdentityMatchPolicy.test.mjs` + `trustOnboardingGate.test.mjs` + `trustOnboardingPolicy.test.mjs` → **344/344 PASS**. Full regression `node --test 'tests/*.test.mjs'` → **649/650** (the same pre-existing `eventAnalyticsWorkbook.test.mjs:93` XLSX `writeBuffer` timing flake, identical signature — ~52.8s against a 20s budget at maximum simulated load, reproduced on this candidate, not a new failure). `npm run build` → clean, zero errors; `/crear-rifa`, `/crear-colecta`, `/crear-evento` confirmed `ƒ` (dynamic/server-rendered) in the build manifest, confirming `getServerSideProps` is active on all 3.

**Self-audit grep** on the full candidate diff against `origin/main` for `payment|webhook|marketplace_fee|argentina|migration|service_role|mp_identity_match\s*=|RIFEX_FEE_RATE`: zero real matches. `git diff --stat origin/main -- src/pages/api/ src/lib/trustIdentityGate.js`: empty — the API layer and the eligibility authority itself are untouched by this promotion, confirming the authoritative server-side gate stays intact independent of this UX-layer change. Grep for `User-Agent|navigator.userAgent|googlebot|bytespider` across the same diff: zero matches — no cloaking introduced.

**Pre-push live smoke on the candidate** (local `next dev` on the exact promoted code, port 3052): anonymous `GET` to `/crear-rifa`, `/crear-colecta`, `/crear-evento` → real `307` to `/login?next=%2F<page>` with minimal bodies (25-28 bytes) — no form HTML in any response. `/registro/continuar`, `/panel/bancos`, `/trust/verificar` → `200` both bare and with `?next=%2Fcrear-rifa`, no crash. A malicious `next=https://evil.com` on `/registro/continuar` still renders `200` (sanitization happens client-side via `sanitizeNextPath` before any `router.replace`, matching the pre-existing pattern — no server-side redirect ever echoes the raw value). `/rifas/crear` (the pre-existing client-side alias) confirmed still `200` with `body{display:none}` and zero occurrences of the real form's field placeholders in the initial HTML — not a new leak, same behavior as before this mission. Regression spot-check on the same running candidate: `/login` and `/register` `200` with real inline hCaptcha (`window.hcaptcha`, 2 occurrences each) and no `captchaGate` references; `captchaGate.js` confirmed absent from the candidate's `src/lib/`; `/mis-iniciativas` (anon) `307`; `/eventos` (public listing) `200`; `/rifas` (legacy redirect) `307`; `/blog` `200` (still gated per its own logic); `/` `200`.
...
**Push**: direct `:main` push blocked at the Claude Code tool-permission layer (as in every prior promotion). Worked around by pushing the commit to `origin/release/rifex-public-surface-final-2026-09-02` (no `:main` suffix), then Rodrigo ran `git fetch origin release/rifex-public-surface-final-2026-09-02` + `git push origin origin/release/rifex-public-surface-final-2026-09-02:main` from his own terminal at `/home/desktop/rifex-frontend-v2`. `origin/main` advanced `39b47f5` → `9124678`, a clean fast-forward. Vercel auto-deployed via the existing GitHub integration; confirmed live via `last-modified` header on `/reglas-iniciativas-premio` (~1 minute old at check time) and cache-busted content checks, not deployment-status trust alone.

**Live PROD smoke, against `rifex.pro`**: all required routes return correct status — public surfaces (`/`, `/login`, `/register`, `/wizard`, `/contacto`, `/confianza`, `/seguridad`, `/reembolsos`, `/reglas-iniciativas-premio`, `/terminos-rifas`, `/blog`, `/robots.txt`, `/sitemap.xml`) `200`; `/rifas` and the 5 auth-boundary routes (`/crear-rifa`, `/crear-colecta`, `/crear-evento`, `/panel`, `/mis-iniciativas`) real `307`s with minimal (18-28 byte) redirect bodies. **Critical checks**: `/reglas-iniciativas-premio` and `/terminos-rifas` — zero matches for the internal legal-review banner (cache-busted). `/contacto` — zero matches for the identity placeholder. `/rifas` — real `307` to `/login?next=%2Fpanel` with `X-Robots-Tag: noindex, nofollow` and the new `Permissions-Policy` header present. Blog — `noindex, nofollow, noarchive` meta confirmed. Home — navbar renders exactly `Eventos`/`Campañas`/`Cómo funciona`/`Ingresar`.

**Crawler test, live**: homepage response byte-identical (same MD5) across default UA, `Googlebot/2.1`, `facebookexternalhit/1.1`, and `TikTokBot` — zero cloaking. JSON-LD present with the real `https://rifex.pro` URL (`Organization` + `WebSite`, no invented facts). All 4 security headers present on the live response. `sitemap.xml` clean of protected/legacy/annex paths; `robots.txt` retains all 14 `Disallow` entries unchanged.

**Payment/Trust/Events/Cumplimiento business logic**: confirmed untouched via `git diff 39b47f5 9124678` — the release's code/test diff touches exactly 12 files (`next.config.mjs`, `Layout.jsx`, `confianza.js`, `contacto.js`, `index.js`, `reglas-iniciativas-premio.js`, `rifas.js`, `terminos-rifas.js`, `wizard.js`, and 3 test files), none inside `payment|checkout|webhook|trust|cumplimiento|api/events|drawWinner|mercadopago` paths. No real financial operations were performed to demonstrate this — evidence is the diff and the passing test suites (including `blogPrivateProd.test.mjs`, PROD-exclusive).

**Status: PROMOTED, DEPLOYED, SMOKED CLEAN.** `origin/main` @ `9124678`. No migrations, no PROD data changes, no real payments, no real emails, no secret changes, no commission-rate change, no Argentina activation, no Trust/Cumplimiento/Events business-logic change. New baseline tag: `v2.6-rifex-prod-public-surface-final`.
...
**Push**: direct `:main` push blocked at the Claude Code tool-permission layer (as in prior promotions). Worked around by pushing the commit to `origin/release/rifex-authux-crawler-prod-2026-09-02` (no `:main` suffix), then Rodrigo ran `git fetch origin release/rifex-authux-crawler-prod-2026-09-02` + `git push origin origin/release/rifex-authux-crawler-prod-2026-09-02:main` from his own terminal at `/home/desktop/rifex-frontend-v2`. `origin/main` advanced `15d7d35` → `e3f298a`, a clean fast-forward. Vercel auto-deployed via the existing GitHub integration; live within ~40s, confirmed by polling `https://rifex.pro/login`'s `<title>` until it read "Ingresar — Rifex" (4 attempts, ~40s).

**Live PROD smoke, against `rifex.pro`**: public surfaces `/`, `/login`, `/register`, `/wizard`, `/eventos` all `200`. The 5 auth-boundary routes (`/crear-rifa`, `/crear-colecta`, `/crear-evento`, `/panel`, `/mis-iniciativas`) each return a real `307` to `/login?next=<path>` with the correct path, and their response bodies are minimal redirect stubs (18-28 bytes) — no form/dashboard HTML leaked to anonymous requests. Navbar on the live homepage renders exactly `Eventos`/`Campañas`/`Cómo funciona`/`Ingresar` (desktop + mobile), zero occurrences of "Crear una iniciativa". Login/Register HTML confirmed: `<title>Ingresar — Rifex</title>` / `<title>Crear cuenta — Rifex</title>`, zero occurrences of "Crea rifas en minutos" / "Regístrate para crear y administrar tus rifas", correct footer cross-links present (`¿Aún no tienes cuenta? Crear cuenta` / `¿Ya tienes cuenta? Ingresar`). Blog (`/blog`) returns `200` with `<meta name="robots" content="noindex, nofollow, noarchive">` in the static HTML and zero post content in that static HTML (client-fetched only after an authenticated session, per its already-certified private behavior) — unchanged. `/rifas` returns `200` with `x-robots-tag: noindex, nofollow` (its already-certified legitimate redirect stub) — unchanged. `sitemap.xml` lists none of the 8 protected/legacy paths checked (`/crear-rifa`, `/crear-colecta`, `/crear-evento`, `/panel`, `/mis-iniciativas`, `/login`, `/register`, `/blog`); `robots.txt` still `Disallow`s all of them and points at the live sitemap. No-cloaking check: homepage response body is byte-identical (same MD5) for a default UA and a `Googlebot/2.1` UA.

**New baseline tag**: `v2.5-rifex-prod-auth-crawler` (the suggested name from the governing prompt fits the repo's real convention, confirmed against the existing `v2.1`..`v2.4` series), annotated, pointing at `e3f298a`, pushed to `origin`.

**Status: PROMOTED, DEPLOYED, SMOKED CLEAN.** `origin/main` @ `e3f298a`. No migrations, no PROD data changes, no real payments, no real emails, no secret changes, no Stage 3, no onboarding progresivo, no MP QUALITY, no C6, no additional SEO work in this release.
...
**Scope determined mechanically, not by inspection**: the file set is the union of `git diff --name-only` over the V4 A1-A7 commit range (`4d59450..37d89d0`) and the Stage 2 commit range (`0075749^..9875ef0`) on `origin/develop`. The full `main..develop` diff had 75 files; 11 were deliberately excluded and left untouched on `main`:
- `src/components/DevBanner.jsx`, `src/lib/captchaGate.js`, and the corresponding hook in `src/pages/_app.js` — DEV-only tooling (banner + hCaptcha bypass); promoting the bypass to PROD would be a real security regression.
- `src/pages/api/blog/index.js`, `src/pages/api/blog/[slug]/index.js`, `src/pages/blog/index.js`, `tests/blogPrivateProd.test.mjs` — Blog was already promoted separately (`2c39587`) with its own PROD-specific implementation; not touched again here.
- 3 `docs/releases/*.md` files and `db/migrations/2026-08-26d5_ar2_country_columns_reconstructed.sql` — exclusive to `main`'s own release history; preserved byte-for-byte.

**Content-level self-audit**: grepped the full diff of the 61 authorized code files for `webhook|paymentEngine|feePolicy|providerRegistry|RIFEX_FEE_RATE|argentina|migration|checkout/mp` — zero real matches in code (the handful of hits were historical prose inside `docs/WOP.md`/`CURRENT_STATE.md`, not code). The one Trust-adjacent change is a read-only addition to `GET /api/rifas/[id]` (`creator_trust_level`, derived from an existing `merchant_gateways` row, no writes, no commission logic) — pre-existing V4 A3 work, already certified in an earlier DEV mission, brought in as part of this expanded scope.

**What this release actually changes on PROD**, at a high level:
- **V4 Public Trust foundation (A1-A7)**: public metadata infra (`publicMetadata.js`, canonical always `rifex.pro`), `robots.txt`/`sitemap.xml` created for the first time on PROD, the public `/rifas` catalog removed (`rifas.js` reduced, no longer a public listing), `TrustBadge`/`TrustPopup` (read-only display of `creator_trust_level`), `noindex` added to every authenticated/private page (`login`, `register`, `panel/*`, `admin/*`, `perfil`, `crear-rifa`, `crear-colecta`, `onboarding/pais`, `registro/continuar`, `reset-password`), and the creation of `cookies.js`, `privacidad.js`, `uso-aceptable.js`, `politica-eventos.js`, `politica-campanas.js`, `reembolsos.js`, `reglas-iniciativas-premio.js`, `terminos-rifas.js`, `confianza.js`, plus rewritten `contacto.js`/`reportar.js` with real backend forms (`api/contacto.js`, `api/reportar.js`).
- **Stage 2 on top of that foundation**: `/terminos` rebuilt as a corporate-only document (no duplicated Privacidad/Cookies summaries, no unreviewed 3-month liability cap), `/seguridad` and `/preguntas-frecuentes` using consistent transversal language, navbar reduced to Eventos/Campañas/Cómo funciona, footer says "Comisión" and includes the community-discovery line, authenticated dropdown without a duplicate "Mis campañas", and `/wizard` representing exclusively Eventos + Campañas (no Rifa/sorteo/premio in the public flow).
- **Rifas as a product is fully preserved**: `crear-rifa.jsx` still works, `/mis-iniciativas` still lists Rifas/Campañas/Eventos, nothing in the ownership/payment/draw logic for Rifas was touched — only its public catalog exposure and public-facing wording changed.

61/61 authorized files verified byte-identical to their `origin/develop` source before commit (zero drift).

**Two real issues found and fixed during candidate validation, before any push:**
- `login.jsx`/`register.jsx`: checking out `develop`'s HEAD content (instead of just the intended V4 A1-A2 diff) silently pulled in a dependency on `src/lib/captchaGate.js` — a helper from the later, unrelated, unauthorized D5-FINAL mission (hCaptcha DEV bypass), breaking the build (`Module not found`). Fixed by resetting both files to `main`'s real, already-working content (inline hCaptcha verification against the pre-existing `/api/verify-captcha`) and applying only the single-line V4 A1-A2 change (`<Layout>` → `<Layout noindex>`). No `captchaGate.js`, `DevBanner.jsx`, or any other D5-FINAL content reached PROD.
- `tests/blogPrivateProd.test.mjs` (a `main`-exclusive test, untouched otherwise) had one assertion asserting `sitemap.xml` must not exist — true before this release, false by design after it. Updated that single assertion to check the invariant that actually matters (`/blog` absent from the real `sitemap.xml`, still `Disallow`'d in `robots.txt`); no product code changed to satisfy it.

**Final validation on the corrected candidate**: `tests/publicAudit.test.mjs` + `tests/blogPrivateProd.test.mjs` 135/135 PASS; full regression 561/562 (the same pre-existing XLSX `writeBuffer` timing flake, identical signature); `npm run build` clean, zero errors.

**Push**: `origin/main` advanced `a2d6a60` → `4c481d8`, a clean fast-forward (pushed by Rodrigo directly from his own terminal after the tool-level push permission was denied twice to the agent; the exact commit and diff were unchanged). Vercel auto-deployed via the existing GitHub integration; live within ~60s, confirmed by `public/sitemap.xml` (previously nonexistent on PROD) returning `200` for the first time.

**PROD smoke, live against `rifex.pro`**: all 16 required public surfaces (`/`, `/eventos`, `/wizard`, `/planes`, `/preguntas-frecuentes`, `/terminos`, `/privacidad`, `/cookies`, `/uso-aceptable`, `/seguridad`, `/cumplimiento`, `/reportar`, `/contacto`, `/politica-eventos`, `/politica-campanas`, `/reembolsos`) return `200`. `/wizard` special check: "Quiero crear un evento"/"Quiero crear una campaña" present, zero matches for rifa/sorteo/premio-specific copy, both CTAs point to the real `/crear-evento`/`/crear-colecta` routes, canonical `https://rifex.pro/wizard`. Canonical spot-checked across 7 surfaces, always `rifex.pro`. `sitemap.xml` lists 17 URLs, none is `/blog`; `robots.txt` has `Disallow: /blog` and points to the new sitemap. Blog re-certified: `/blog` returns `200` (auth-gated, per its already-certified private behavior) with `x-robots-tag: noindex, nofollow, noarchive`, no `/blog` link in the public footer/nav. Rifas re-certified: `/crear-rifa` and `/mis-iniciativas` both `200` (untouched, fully preserved); the public `/rifas` catalog itself now returns `200` with `x-robots-tag: noindex, nofollow` and zero individual-raffle links in its markup (V4 A6's catalog removal, confirmed live for the first time on PROD). Footer "Conoce más productos de Rifex" confirmed present.

**Status: PROMOTED, DEPLOYED, SMOKED CLEAN.** `origin/main` @ `4c481d8`. No migrations, no Payment Engine changes, no webhook changes, no Trust backend logic changes, no commission-rate changes, no Argentina activation, no Etapa 3 work in this release. New baseline tag: `v2.4-rifex-prod-public-trust`.

---

## RIFEX BLOG PRIVATE — PROD PROMOTION (2026-09-01) — Blog stops being a public surface on PROD

`origin/main`/PROD advanced `b9a9798` → `2c39587`, a clean fast-forward push (no merge, no force), tagged `v2.3-rifex-prod-blog-private`. This promotes exactly one thing: the Blog feature (footer link, 4 pages, 2 read APIs) becomes auth-gated in PROD — nothing else moved. In particular, none of the V4 Public Trust A+B infrastructure (`robots.txt`, `sitemap.xml`, `publicMetadata.js`, canonical/OG metadata plumbing) went to PROD; it remains `develop`-only.

The equivalent of `develop`'s Blog Private commit (`bf5745b`) could not be cherry-picked directly — its diff context assumes `Layout.jsx`'s V4-rewritten state, which `main` never received. Each affected file was instead diffed against `main`'s real current content to confirm zero unrelated drift (Blog's own files predate V4 and were byte-identical to `develop`'s pre-patch base), then the same functional patch was manually re-applied on top of `main`'s actual files. `Layout.jsx`'s change was reduced to exactly the single footer `Blog` link removal.

Target state confirmed: footer/navbar/authenticated-menu all without a Blog link (the authenticated-menu addition from `develop`'s `202889a` was reverted there before this promotion and never reached PROD), `noindex, nofollow, noarchive` on all 4 pages, both read APIs return `401 missing_auth` anonymously, anonymous direct access redirects to `/login?next=...`, authenticated access unaffected, nothing deleted. 14/14 new scoped tests pass, full regression 440/441 (the one failure is the pre-existing XLSX `writeBuffer` timing flake, unrelated), build clean. Scope self-audit: `git diff --stat origin/main` showed exactly 8 files (the 7 Blog-related files + the new test file); a grep for payment/webhook/Trust/commission keywords across the full diff returned zero matches. PROD smoke confirmed live post-deploy: footer clean, `/blog` serves `noindex`, both APIs 401 anonymously, general site (`/eventos`, `/crear-colecta`, `/crear-evento`, `/login`, `/terminos`, `/planes`) unaffected, `POST`-only webhook endpoint still rejects `GET` with `405`.

Full detail: `docs/releases/RIFEX_BLOG_PRIVATE_PROD_PROMOTION_2026-09-01.md`. Per Rodrigo's combined authorization, PROD authorization ends here — ETAPA 2 (Identidad Pública + Políticas) begins immediately after on `develop` only, autonomously, with no further PROD writes.

---

## RIFEX CONTROLLED PROD RELEASE (2026-08-31) — Cumplimiento V1 + Onboarding/Bancos-MP + Events Capacity/Live Attendance + Home V1 promoted to PROD

`origin/main`/PROD advanced `e7311c1` → `5145d91`, a clean fast-forward push (no merge, no force). This closes a real gap in the two entries below: between the `RIFEX FULL PROD RELEASE (2026-08-30)` documented right below (`3f3d6c4` → `5c15624`) and this release, `main` had already advanced a further step to `e7311c1` via a separate release branch (`release/rifex-full-prod-2026-08-30`) that promoted TRUST identity-verification UI/endpoints, an earlier onboarding/bancos version, `/trust/verificar`, `/panel/admin/trust/*`, `/seguridad`, a static `/cumplimiento` roadmap page, and AR2 country columns — documented only in `docs/releases/RIFEX_FULL_PROD_RELEASE_2026-08-30.md` (a file that exists solely on `main`, was never merged back to `develop`, and is therefore invisible from this file's own history further down). Any earlier note in this document or in `docs/CURRENT_STATE.md`/`docs/handover/NUEVA_SESION_PROMPT.md` naming `main` as `3f3d6c4` or `5c15624` is historical, not current — treat live Git state (`git rev-parse origin/main`) as authoritative over any WOP narrative.

This release promoted exactly 4 certified `develop` blocks, file-exact (`git checkout origin/develop -- <path>` per block, one commit per block — never a merge, never the raw 60+ commit range, since `main` and `develop` diverged well before this promotion and a commit-range diff would have been noisy/misleading against the true 65-file tree diff):

- **Cumplimiento V1** (C1/C3/C4/C5): post-raffle compliance case tracking — foundation schema, communications, timeline/escalation scheduler, admin review. 4 additive migrations (`2026-08-30_cumplimiento{1,3,4,5}_*.sql`). No automatic email/cron was activated as part of this promotion — the cron endpoint exists but was not manually invoked during release verification.
- **Onboarding + Bancos/MP**: neutral onboarding screen, `/panel/bancos` state machine (disconnected/pending/validated/mismatch/unavailable), server-side "Verificar cuenta" revalidation endpoint, Stripe visual-only catalog card (`enabled:false`, no real integration — confirmed absent from the repo by explicit negative-assertion tests). Argentina remains `enabled:false` in every environment including PROD, untouched by this release.
- **Events Capacity + Live Attendance** (EVENT-8): `events.capacity`, the aforo invariant (`_check_event_capacity()` + 2 triggers), live attendance in the scanner derived solely from `event_tickets.used_at`, never a mutable counter. 1 additive migration (`2026-08-30_event8_capacity_live_attendance.sql`). The one real historical PROD event received `capacity=NULL`, never an invented value.
- **Home V1**: full public Home redesign around Eventos/Entradas digitales/Campañas de recaudación, real photo hero (`public/images/hero/rifex-hero-events.png`), premium capability cards. Rifas-first messaging removed from Home only — `/rifas` itself is untouched and fully functional.

All 5 PROD migrations applied in strict dependency order, each preceded by a live drift re-check and followed by a targeted schema verification query. Post-migration integrity confirmed: RLS enabled on every new table, zero grants to `anon`/`authenticated`/`public` on new tables, all 6 pre-existing financial/inventory RPCs still present, financial table row counts intact (`raffles`=7, `tickets`=420, `payments`=4, unchanged before/after). Release-branch autoaudit against `origin/main=e7311c1` confirmed exactly 65 files changed, no F-category leakage (no `DevBanner.jsx`/`captchaGate.js`/`_app.js` DEV wiring/`login.jsx`/`register.jsx` changes), both main-exclusive files preserved untouched (`db/migrations/2026-08-26d5_ar2_country_columns_reconstructed.sql`, `docs/releases/RIFEX_FULL_PROD_RELEASE_2026-08-30.md`). `npm test`: 426/427 (the 1 failure is the pre-documented XLSX `writeBuffer` timing flake at the 20k/20k/20k/500 stress ceiling — reproduced twice with the identical signature, not a regression). `npm run build`: clean.

Deployed via the existing GitHub → Vercel auto-deploy integration (no manual `vercel deploy`), Vercel project `rifex-frontend-v2`, aliased to `rifex.pro`/`www.rifex.pro`. Verified via real HTTP content (not deployment-status trust alone): Home hero copy, capability-card markup, and page `<title>` match the promoted code exactly; `/cumplimiento`, `/panel/bancos`, `/eventos`, `/rifas`, and a spot-checked historical event page all return real 200s with correct content; `/api/events/[id]/check-in` correctly returns `401` unauthenticated rather than erroring. No real payment, fixture, test email, manual cron invocation, or real check-in was performed during release verification.

Known non-blockers carried forward, none fixed opportunistically in this release: XLSX `writeBuffer` timing flake under maximum stress load (pre-existing, documented above); `/cumplimiento` is roadmap copy vs. the now-real Cumplimiento engine — a semantics gap, not a functional defect, deferred to a future PUBLIC TRUST/POLICIES mission; `merchant_gateways` broad anon/authenticated grants mitigated by RLS (pre-existing, unrelated to this release); MP Quality 89/100 (pre-existing); Difusión/redes V4 not implemented; legal/policy review still pending a Chilean lawyer. Difundir iniciativa, TXT V4, MP QUALITY 100 work, new policies, Argentina reactivation, Stripe real integration, and any uncertified `develop` work were explicitly excluded from this promotion and remain on `develop` only.

Full evidence and the 30-point final report: `docs/releases/RIFEX_CONTROLLED_PROD_RELEASE_2026-08-31.md`.
