...
```text
NEXT: EVENT-7 — not scoped, not authorized. Urgent, independent of Events, PC-de-escritorio-only: verify/fix create_tickets_for_raffle grants in PROD (see docs/handover/HANDOVER_NOTEBOOK_TO_DESKTOP_2026-08.md, section 5). TRUST-1, TRUST-2 and TRUST-3A are DONE in DEV, and the Mercado Pago identity-match correction is DONE in DEV too (code + migrations applied + pushed + deployed, see checkpoints above) — TRUST-3B/TRUST-4 onward (OCR/KYC, biometrics, organizations, appeals, retention jobs, mandatory activation) not authorized, not started. Urgent real follow-up: confirm with real Mercado Pago credentials whether /users/me actually returns identification for Chile (docs/trust/MP_IDENTITY_MATCH_AUDIT.md) — the match logic works defensively either way, but nobody has proven the real-world behavior yet. Human UI testing of TRUST-1/TRUST-2/TRUST-3A/Mercado-Pago-onboarding is scheduled for this weekend with Rodrigo rested — nothing further should be built on top of this until that happens. A new Events backlog item was recorded (docs/events/EVENTS_BACKLOG.md — downloadable per-event promotional QR, distinct from the per-ticket access QR) — documentation only, not scoped, not part of EVENT-7. PROD promotion of Events and of Trust — a business decision, not a technical audit conclusion — reserved for Rodrigo.
```
