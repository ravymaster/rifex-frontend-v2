...
## Finding 8 — Two parallel layout/nav systems, inconsistently applied

`src/components/Layout.jsx` is the real, actively-used shared header/nav/footer wrapper — imported by 12 pages (`index.js`, `planes.js`, `perfil.js`, `contacto.js`, `rifas.js`, `register.jsx`, `chat/[raffleId].js`, `blog/index.js`, `blog/[slug].js`, `panel/index.js`, `panel/bancos.js`, `panel/mercado-pago.js`). It is pure presentation — no Supabase calls, no business logic, confirmed by full read — genuinely safe to restyle in isolation. It contains the hardcoded 14-item nav array the user described ("home tiene todos los links en la navbar porque así me era más fácil trabajar") — including debug/internal entries like `Chat (demo)` → `/chat/123` and `MP Setup` → `/panel/mercado-pago` sitting alongside real user-facing links.

`src/components/Header.jsx` is a **second, separate** header/nav implementation — confirmed orphaned: `grep` shows it is imported by nothing (only self-references within its own file). Add it to the orphaned-component list from Finding 5.

**The bigger finding**: 12 other pages import neither `Layout.jsx` nor `Header.jsx` and render with **no shared site navigation at all** — `terminos.js`, `reset-password.jsx`, `login.jsx`, `icons.js`, `crear-rifa.jsx`, `protegida.jsx`, `404.js`, `checkout/index.js`, `rifas/crear.jsx`, and critically **`rifas/[id].jsx`** — the raffle detail / checkout entry page, arguably the single most-visited and most important page in the product. A visitor there has no way back to the rest of the site except whatever ad-hoc inline links that specific page happens to render (confirmed: `/panel`, the two broken links from Finding 7, `/terminos`) or the browser back button.

**What this means for scope**: "redesign the home page" and "fix the navbar" are not the same task. `index.js` + `Layout.jsx` can be redesigned today in complete isolation — zero business logic, confirmed safe. But making navigation *consistent site-wide* is a materially larger scope: it would eventually mean adding `Layout` (or its replacement) to pages that currently have none, including `rifas/[id].jsx` — the highest-risk file in this entire audit (Finding 1). Adding a wrapper around a page without touching its internal logic is lower-risk than editing that logic directly, but it is not zero-risk (layout/CSS conflicts, z-index, the intro modal and payment-return banner already fighting for screen space on that page) and should not be assumed safe by extension of "the home page is fine."
