...
Sistema transversal de onboarding, identidad, verificación, prevención de fraude, administración, denuncias, reputación basada en operaciones reales, y expansión futura — diseñado en su mayoría, **no implementado**, salvo TRUST-1, TRUST-2, TRUST-3A y la corrección canónica de Mercado Pago (ver notas abajo).

> **Actualización posterior a la fecha de este handoff (2026-08-26)**: TRUST-1 (onboarding universal) quedó **COMPLETO en `rifex-dev`** — Rodrigo autorizó expresamente aplicar la migración, crear fixtures desechables, push a `origin/develop` y deploy DEV, y las cuatro acciones se ejecutaron y verificaron (migración `db/migrations/2026-08-26e_trust1_onboarding.sql` aplicada y confirmada con RLS activo y cero grants; flujo real probado en vivo contra `rifex-frontend-main` con fixtures `@example.com` borradas después, cero residuos; Security Advisor sin hallazgos nuevos; commit `6333044` empujado y desplegado). Ver `docs/trust/TRUST_IMPLEMENTATION_ROADMAP.md`, sección TRUST-1, y `docs/WOP.md`, "TRUST-1 checkpoint", para el detalle exacto.
>
> **Actualización posterior (2026-08-27)**: TRUST-2 (identidad básica declarada: RUT chileno con dígito verificador + requisito de edad 18+) también quedó **COMPLETO en `rifex-dev`**, en una misión autónoma pre-autorizada de punta a punta (Rodrigo llegó agotado tras 500 km de viaje y pidió explícitamente no ser consultado ese día — pruebas humanas de interfaz quedaron agendadas para el fin de semana siguiente, con Rodrigo descansado). Migración `db/migrations/2026-08-27_trust2_identity.sql` aplicada y verificada; un bug real (`.update()` en vez de `.upsert()`, fallaba en silencio si el usuario no tenía fila previa en `trust_onboarding`) se encontró adversarialmente en DEV y se corrigió en la misma sesión; un `409 rut_conflict` real se confirmó contra el índice único de Postgres entre dos cuentas distintas. Commit `5fa5bd4` empujado y desplegado. Ver `docs/trust/TRUST_IMPLEMENTATION_ROADMAP.md`, sección TRUST-2, y `docs/WOP.md`, "TRUST-2 checkpoint", para el detalle exacto.
>
> **Actualización posterior (2026-08-27, misma fecha)**: TRUST-3A (verificación documental de identidad, revisión manual, exclusivamente personas naturales con cédula chilena — sin OCR/biometría/organizaciones) también quedó **COMPLETO en `rifex-dev`**, misma modalidad de misión autónoma pre-autorizada de punta a punta. Dos migraciones (`db/migrations/2026-08-27b_trust3a_identity_verification.sql` y `2026-08-27c_trust3a_fix_user_deletion_fks.sql`) aplicadas y verificadas, más un bucket privado nuevo en Storage (`trust-documents`, confirmado default-deny en vivo). Dos bugs reales encontrados adversarialmente y corregidos en la misma sesión — ver `docs/trust/TRUST_IMPLEMENTATION_ROADMAP.md`, sección TRUST-3A, para el detalle completo (incluye por qué el trigger de inmutabilidad del audit log inicialmente impedía borrar cualquier cuenta con historial, y cómo se corrigió). `identity_verified`/`age_verified` ya son columnas reales, pero la activación de "identidad verificada obligatoria para publicar" sigue apagada — decisión de negocio pendiente. Commit `f2f018b` empujado y desplegado. Ver `docs/WOP.md`, "TRUST-3A checkpoint", para el detalle exacto.
>
> **Actualización posterior (2026-08-27/28, sesión nocturna, Rodrigo dormido)**: corrección canónica hacia adelante — **Mercado Pago como control principal, onboarding simplificado**, también **COMPLETA en `rifex-dev`**. No se revirtió ningún commit ni migración — solo aditivo/correctivo. Fecha de nacimiento eliminada por completo (0 filas reales confirmadas antes de borrar), reemplazada por una declaración booleana versionada. El selector `account_type` reemplazado por `person_name`/`organization_name` derivado server-side. Mercado Pago pasa a exigirse, junto con la coincidencia de titularidad contra el RUT declarado, como parte del cierre real del onboarding — con un fallback honesto (`unavailable`, nunca bloquea) porque **no se pudo confirmar en vivo si Mercado Pago realmente entrega el RUT para Chile** (sin credenciales de una app de Mercado Pago en este entorno, documentación oficial bloqueada — ver `docs/trust/MP_IDENTITY_MATCH_AUDIT.md`). TRUST-3A sigue como respaldo excepcional. Migración `db/migrations/2026-08-28_mp_identity_match_onboarding_correction.sql` aplicada. Commit `0cc59dc` empujado y desplegado. Ver `docs/WOP.md`, sección de corrección canónica, para el detalle exacto.
>
> **Actualización posterior (2026-08-29)**: auditoría adversarial autónoma de solo lectura (sin correcciones aplicadas) sobre TRUST-1/2/3A y el onboarding Mercado Pago. **Encontró un fail-open CRÍTICO real y demostrado, IMPORTANTE leer antes de seguir**: `assertCreatorEligible` (`src/lib/trustIdentityGate.js`) deja pasar `mp_identity_match=NULL` exactamente igual que `'matched'` — un estado alcanzable en la práctica si la resolución del match falla en silencio tras conectar Mercado Pago. Confirmado con una prueba local aislada, permanente en `tests/trustIdentityGate.test.mjs`. Detalle completo, severidades y correcciones mínimas propuestas (ninguna aplicada todavía) en `docs/trust/TRUST_MP_ADVERSARIAL_AUDIT_2026-08.md`. Veredicto: `GO CON CONDICIONES` — **corregir el hallazgo crítico antes de cualquier prueba humana con cuentas reales de Mercado Pago**.
>
> Si esto se retoma desde Santiago, **lee primero `docs/trust/TRUST_MP_ADVERSARIAL_AUDIT_2026-08.md` completo** antes de tocar nada de Trust/Mercado Pago. No hay ninguna migración de TRUST-1/TRUST-2/TRUST-3A/Mercado-Pago pendiente de aplicar — todas ya están en `rifex-dev`, pero el gate real tiene un fail-open sin corregir. Verifica si las pruebas humanas de interfaz (agendadas para el fin de semana del 2026-08-27) ya se hicieron, y si alguien ya confirmó con credenciales reales de Mercado Pago el comportamiento real de `/users/me` para Chile. El siguiente paso de Trust, si Rodrigo lo autoriza, es corregir los hallazgos de la auditoría — TRUST-3B o TRUST-4 (OCR/KYC, biometría, organizaciones, apelaciones, retención automática) siguen sin iniciar.
