...
>
> Si esto se retoma desde Santiago, no hay ninguna migración de TRUST-1/TRUST-2/TRUST-3A/Mercado-Pago pendiente de aplicar — todas ya están en `rifex-dev`. Verifica primero si las pruebas humanas de interfaz (agendadas para el fin de semana del 2026-08-27) ya se hicieron antes de construir nada encima, y si alguien ya confirmó con credenciales reales de Mercado Pago el comportamiento real de `/users/me` para Chile. El siguiente paso de Trust, si Rodrigo lo autoriza, es TRUST-3B o TRUST-4 (OCR/KYC, biometría, organizaciones, apelaciones, retención automática), todavía sin iniciar.
