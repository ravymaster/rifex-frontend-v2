...
> **2026-09-03 — PROMOVIDO A PROD.** Autorizado explícitamente por Rodrigo ("GO A PROD"). `origin/main` avanzó desde `37f0820` (tag `v2.6-rifex-prod-public-surface-final`) promoviendo exactamente este trabajo DEV certificado (`origin/develop` @ `b996893`). Ver `docs/WOP.md`, "RIFEX PROGRESSIVE ONBOARDING — PROD PROMOTION (2026-09-03)" para el detalle completo de la promoción (clasificación de archivos, tests sobre el candidate real, push, deploy, smoke en `rifex.pro`). El contenido debajo describe la misión DEV original tal como fue certificada; queda como registro histórico de esa fase, no como estado "todavía no en PROD".
