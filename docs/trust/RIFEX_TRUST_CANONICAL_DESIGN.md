...
> **Actualización — TRUST-1, TRUST-2 y TRUST-3A implementados y desplegados en `rifex-dev`.** TRUST-1 (onboarding universal), TRUST-2 (RUT chileno + edad 18+ declarados) y TRUST-3A (verificación documental manual — solo personas naturales, solo cédula chilena, solo revisión humana, sin OCR/biometría) están completos, probados en vivo y aplicados en DEV. `identity_verified`/`age_verified` ya son columnas reales, escribibles únicamente por una aprobación administrativa. La ACTIVACIÓN de "identidad verificada obligatoria para crear/publicar" sigue apagada por defecto (`isIdentityVerificationRequiredForCreators() === false`) — es una decisión de negocio pendiente, no un efecto de haber construido TRUST-3A. El resto de este documento (TRUST-3B en adelante: OCR/KYC automatizado, biometría, organizaciones/RUT tributario/representantes, motor de riesgo, apelaciones) sigue siendo diseño puro, no implementado. Ver `docs/trust/TRUST_IMPLEMENTATION_ROADMAP.md`, secciones TRUST-1/TRUST-2/TRUST-3A, para el detalle real de lo ya construido.
